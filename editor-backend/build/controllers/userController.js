...
export const createData = async (req, res) => {
    try {
        const { title, description, links, imageUrl } = req.body;
        if (!req.user?.id) {
        ...
        if (imageUrl) {
            if (Array.isArray(imageUrl)) {
                uploadedImageUrls = [...uploadedImageUrls, ...imageUrl.map(String)];
            }
            else if (typeof imageUrl === "string") {
                const trimmedImageUrl = imageUrl.trim();
                if (trimmedImageUrl.startsWith("[") && trimmedImageUrl.endsWith("]")) {
                    try {
                        const parsed = JSON.parse(trimmedImageUrl);
                        uploadedImageUrls = [
                            ...uploadedImageUrls,
                            ...(Array.isArray(parsed) ? parsed.map(String) : [trimmedImageUrl]),
                        ];
                    }
                    catch {
                        uploadedImageUrls = [...uploadedImageUrls, trimmedImageUrl];
                    }
                }
                    ...
                    uploadedImageUrls = [
                        ...
                        ...(trimmedImageUrl.includes(",")
                            ? trimmedImageUrl.split(",").map(item => item.trim())
                            : [trimmedImageUrl]),
                    ];
                }
            }
